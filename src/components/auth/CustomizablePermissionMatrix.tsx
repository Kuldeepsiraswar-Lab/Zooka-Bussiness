...
export const CustomizablePermissionMatrix: React.FC = () => {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-white rounded-3xl border border-slate-200 shadow-xs p-5 flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <div className="flex items-center gap-2">
                <h3 className="font-bold text-slate-900 text-base">Customizable Permission Matrix</h3>
                {isCurrentUserAdmin ? (
                  <span className="px-2 py-0.5 text-[10px] font-bold bg-emerald-50 text-emerald-700 border border-emerald-200 rounded-full flex items-center gap-1">
                    <ShieldCheck className="w-3 h-3 text-emerald-600" /> Admin Live Control
                  </span>
                ) : (
                  <span className="px-2 py-0.5 text-[10px] font-bold bg-slate-100 text-slate-600 border border-slate-200 rounded-full flex items-center gap-1">
        ...
        <div className="flex flex-wrap items-center gap-2.5">
          ...
          {isCurrentUserAdmin && (
            <button
              ...
              className="px-3 py-2 text-xs font-bold text-slate-600 hover:text-rose-600 hover:bg-rose-50 rounded-xl border border-slate-200 transition-all flex items-center gap-1.5 cursor-pointer shadow-2xs"
      ...
      {/* Filter & Toolbar */}
      <div className="bg-slate-50/90 rounded-2xl border border-slate-200 p-3 flex flex-col sm:flex-row items-center justify-between gap-3">
        <div className="flex flex-wrap items-center gap-2.5 w-full sm:w-auto">
          ...
          <div className="relative w-full sm:w-64">
            ...
            <input
              ...
              className="w-full pl-9 pr-3 py-1.5 text-xs rounded-xl border border-slate-200 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-600 bg-white"
            />
          </div>
        ...
        <div className="flex items-center gap-3 w-full sm:w-auto justify-between sm:justify-end">
          <button
            ...
            className="px-3 py-1.5 bg-white hover:bg-slate-100 text-slate-700 font-bold text-xs rounded-xl border border-slate-200 transition-all flex items-center gap-1 cursor-pointer shadow-2xs"
          >
          ...
          <table className="w-full text-left text-xs border-collapse">
            ...
            <thead>
              <tr className="bg-slate-100/90 border-b border-slate-200 text-slate-700 font-bold">
                <th className="py-4 px-4 min-w-[280px] sticky left-0 bg-slate-100/95 backdrop-blur-xs z-20 border-r border-slate-200">
                ...
                {matrixMode === 'roles' && rolesToDisplay.map(role => {
                  ...
                  return (
                    <th key={role} className="py-3.5 px-3 min-w-[170px] text-center border-r border-slate-200/80 bg-slate-50/70">
                        ...
                        {isCurrentUserAdmin && (
                          <div className="flex items-center gap-1 mt-0.5">
                            <button
                              ...
                              className="px-1.5 py-0.5 text-[9px] font-bold text-emerald-700 hover:bg-emerald-100 bg-emerald-50 rounded border border-emerald-200 cursor-pointer transition-colors"
                            ...
                            <button
                              ...
                              className="px-1.5 py-0.5 text-[9px] font-bold text-blue-700 hover:bg-blue-100 bg-blue-50 rounded border border-blue-200 cursor-pointer transition-colors"
                            ...
                            <button
                              ...
                              className="px-1.5 py-0.5 text-[9px] font-bold text-rose-700 hover:bg-rose-100 bg-rose-50 rounded border border-rose-200 cursor-pointer transition-colors"
                ...
                {matrixMode === 'users' && usersToDisplay.map(user => {
                  ...
                  return (
                    <th key={user.id} className="py-3.5 px-3 min-w-[170px] text-center border-r border-slate-200/80 bg-slate-50/70">
                      <div className="flex flex-col items-center gap-1.5">
                        <div className="flex items-center gap-2">
                          ...
                          <div className="text-left">
                            <div className="font-bold text-xs text-slate-900 truncate max-w-[110px]">{user.name}</div>
              ...
              {filteredModules.map(mod => {
                ...
                return (
                  <React.Fragment key={mod.key}>
                    {/* MODULE MASTER ROW */}
                    <tr className="bg-slate-50/90 font-semibold text-slate-900 hover:bg-slate-100/80 transition-colors">
                      <td className="py-3 px-4 sticky left-0 bg-slate-50/95 backdrop-blur-xs z-10 border-r border-slate-200">
                        <div className="flex items-center justify-between">
                          <button
                            onClick={() => toggleModuleExpand(mod.key)}
                            className="flex items-center gap-2.5 text-left cursor-pointer group flex-1"
                          >
                            <div className="p-1.5 rounded-xl bg-white border border-slate-200 text-indigo-600 shadow-2xs group-hover:scale-105 transition-transform">
                              <IconComponent className="w-4 h-4" />
                            </div>
                            <div>
                              <div className="font-bold text-slate-900 text-xs flex items-center gap-1.5">
                                <span>{mod.label}</span>
                                <span className="text-[10px] font-mono text-slate-400">({mod.actions.length})</span>
                              </div>
                              <div className="text-[11px] text-slate-500 font-normal leading-tight line-clamp-1">
                                {mod.description}
                              </div>
                            </div>
                          </button>
                          ...
                          <button
                            onClick={() => toggleModuleExpand(mod.key)}
                            className="p-1 text-slate-400 hover:text-slate-700 cursor-pointer ml-2"
                          >
                      ...
                      {matrixMode === 'roles' && rolesToDisplay.map(role => {
                        ...
                        return (
                          <td key={role} className="py-2.5 px-3 text-center align-middle border-r border-slate-200/80">
                            <button
                              ...
                              className={`w-full py-1.5 px-2.5 rounded-xl text-xs font-bold transition-all flex items-center justify-center gap-1.5 ${
                                ...
                                isFull
                                  ? 'bg-emerald-100/90 text-emerald-800 border border-emerald-300 shadow-2xs'
                                  : isNone
                      ...
                      {matrixMode === 'users' && usersToDisplay.map(user => {
                        ...
                        return (
                          <td key={user.id} className="py-2.5 px-3 text-center align-middle border-r border-slate-200/80">
                            <div
                    ...
                    {isExpanded && mod.actions.map(act => (
                      <tr key={`${mod.key}-${act.key}`} className="hover:bg-slate-50/60 transition-colors">
                        {/* Action Label and Tooltip */}
                        <td className="py-2.5 px-4 pl-11 sticky left-0 bg-white/95 backdrop-blur-xs z-10 border-r border-slate-200">
                          <div>
                        ...
                        {matrixMode === 'roles' && rolesToDisplay.map(role => {
                          ...
                          return (
                            <td key={role} className="py-2 px-3 text-center align-middle border-r border-slate-200/80">
                              <button
                                type="button"
                                disabled={!isCurrentUserAdmin}
                                ...
                                className={`group inline-flex items-center gap-1.5 px-3 py-1 rounded-xl text-xs font-bold transition-all select-none ${
                                  ...
                                  isEnabled
                                    ...
                                    : 'bg-slate-100 text-slate-400 border border-slate-200 hover:bg-slate-200/80'
                                }`}
                              ...
                              >
                                {isEnabled ? (
                                  ...
                                  <>
                                    <div className="w-4 h-4 rounded-full bg-slate-300 text-slate-600 flex items-center justify-center shrink-0">
                        ...
                        {matrixMode === 'users' && usersToDisplay.map(user => {
                          ...
                          return (
                            <td key={user.id} className="py-2 px-3 text-center align-middle border-r border-slate-200/80">
                              <button
                                type="button"
                                disabled={!isCurrentUserAdmin}
      ...
      {showResetConfirmModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-2 sm:p-4 md:p-6 bg-slate-900/60 backdrop-blur-xs animate-in fade-in duration-100 overflow-y-auto modal-overlay">
          <div className="bg-white rounded-2xl sm:rounded-3xl p-4 sm:p-6 max-w-[96vw] sm:max-w-md w-full shadow-2xl border border-slate-200 space-y-4 max-h-[95dvh] sm:max-h-[90dvh] overflow-y-auto modal-content-scroll my-auto animate-in zoom-in-95 duration-100">
            <div className="flex items-center gap-3">
              <div className="w-11 h-11 rounded-2xl bg-amber-100 text-amber-600 flex items-center justify-center shrink-0">
                <RotateCcw className="w-5 h-5" />
              </div>
              <div>
                <h3 className="text-base font-bold text-slate-900">Reset Permission Matrix?</h3>
                <p className="text-xs text-slate-500">Restore role access policies back to standard RBAC defaults.</p>
              </div>
            </div>
