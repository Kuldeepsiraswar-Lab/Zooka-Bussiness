...
export const UsersAndRolesView: React.FC = () => {
  ...
  const matrixModules: { key: keyof UserPermissions; label: string; description: string }[] = [
    { key: 'dashboard', label: 'Dashboard & KPIs', description: 'Business overview, charts & financial metrics' },
    { key: 'invoices', label: 'Invoices & Billing', description: 'Tax invoices, estimates, credit notes & IRN' },
    { key: 'pos_billing', label: 'POS Counter Billing', description: 'Fast retail checkout & counter slips' },
    { key: 'inventory', label: 'Inventory & Stock', description: 'Items, batches, purchase costs & adjustments' },
    { key: 'parties', label: 'Customers & Vendors', description: 'Party directory, statements & credit limits' },
    { key: 'purchases', label: 'Purchases & Expenses', description: 'Vendor bills, ITC claim & operational costs' },
    { key: 'payments', label: 'Payments & Receipts', description: 'Payment In, Out & Bank transfers' },
    { key: 'accounting', label: 'Accounting & Journals', description: 'General Ledger, P&L, Balance sheet & Trial Balance' },
    { key: 'gst_returns', label: 'GST & Tax Registers', description: 'GSTR-1, GSTR-3B, Sale & Purchase registers' },
    { key: 'settings', label: 'Company Settings', description: 'GSTIN, Bank A/C, Users & Backup/Restore' },
  ];

  const standardRoles: RoleType[] = ['ADMIN', 'ACCOUNTANT', 'SALESPERSON', 'INVENTORY_MANAGER', 'AUDITOR'];
        ...
        <div className="space-y-4">
          <div className="bg-white rounded-3xl border border-slate-200 shadow-xs overflow-hidden">
            <div className="p-5 border-b border-slate-200 bg-slate-50 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
              <div>
                <h3 className="font-bold text-slate-900 text-sm">System Roles Permission Grid</h3>
                <p className="text-xs text-slate-500">
                  Comprehensive action-level access policies governing financial records, tax returns, stock movements, and invoicing.
                </p>
              </div>
              <div className="flex items-center gap-2">
                ...
                <span className="flex items-center gap-1 text-[11px] font-semibold text-slate-600 bg-slate-100 px-2 py-1 rounded-md border border-slate-200">
              ...
              <table className="w-full text-left text-xs border-collapse">
                <thead>
                  <tr className="bg-slate-100/80 border-b border-slate-200 text-slate-700 font-bold">
                    <th className="py-3.5 px-4 min-w-[220px]">Application Module</th>
                    {standardRoles.map(role => {
                      const meta = ROLE_DEFINITIONS[role];
                      return (
                        <th key={role} className="py-3.5 px-3 min-w-[130px] text-center">
                          <div className={`inline-block px-2 py-0.5 text-[10px] uppercase font-extrabold rounded-md border ${meta.badgeBg} ${meta.badgeText}`}>
                            {meta.name}
                          </div>
                        </th>
                      );
                ...
                <tbody className="divide-y divide-slate-200">
                  {matrixModules.map(mod => (
                    <tr key={mod.key} className="hover:bg-slate-50/70 transition-colors">
                      ...
                      {standardRoles.map(role => {
                        const perms = ROLE_DEFINITIONS[role].defaultPermissions[mod.key] as any;
                        const hasView = !!perms?.view;
                        const hasCreate = !!perms?.create || !!perms?.createSale || !!perms?.createProduct || !!perms?.createBill || !!perms?.createJournal || !!perms?.recordPaymentIn;
                        const hasFull = Object.values(perms || {}).every(Boolean);

                        return (
                          <td key={role} className="py-3 px-3 text-center align-middle">
                            {hasFull ? (
                              <div className="inline-flex items-center justify-center w-7 h-7 rounded-xl bg-emerald-100 text-emerald-700 shadow-2xs font-bold">
                                <Check className="w-4 h-4 stroke-[3]" />
                              </div>
                            ) : hasView ? (
                              <div className="inline-flex flex-col items-center">
                                <span className="px-2 py-0.5 text-[9px] font-bold bg-blue-50 text-blue-700 rounded-md border border-blue-200">
                                  {hasCreate ? 'View & Create' : 'View Only'}
                                </span>
                              </div>
                            ) : (
                              <div className="inline-flex items-center justify-center w-7 h-7 rounded-xl bg-slate-100 text-slate-400">
